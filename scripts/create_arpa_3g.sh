if [ $# -lt 4 ]; then
        echo "Usage: sh create_arpa.sh model-dir srilm bigramNNLM gpu'n' "
...
echo "Combining 1gram and 2-gram from SRILM and 3-grams from NNLM.." 
...
tail_size=`grep -n '3-grams' $srilm | cut -d ':' -f1`
head_size=`grep -n '2-grams' $srilm | cut -d ':' -f1`
head_size=`expr $head_size`
tail_size=`expr $tail_size - 2`
tot=`expr $tail_size - $head_size`
echo "2gram:" $head_size $tail_size $tot
#cat $srilm | head -$tail_size | tail -$tot > $model/allphrases.2g.lm
