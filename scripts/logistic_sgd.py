...
class LogisticRegression(object):
    ...
    def __init__(self, input, n_in, n_out,W=None,b=None,W2=None,b2=None):
	...
	def symbolic_softmax(x):
        ...
        # initialize the baises b as a vector of n_out 0
	...
	'''if W2 is None:
            self.W2 = theano.shared(value=numpy.zeros((n_in, n_out/2),
                                                      dtype=theano.config.floatX),
                                    name='W', borrow=True)
        else:
        ...
        if b2 is None:
            self.b2 = theano.shared(value=numpy.zeros((n_out/2,),
                                                  dtype=theano.config.floatX),
                                    name='b', borrow=True)
        else:
        ...
        #self.p_y_given_x = [T.nnet.softmax(T.dot(self.input_split[i], self.W ) + self.b) for i in range(2)] #Ankur
	...
	#self.lin_output2 = T.exp(T.dot(input, self.W2) + self.b2)
        ...
        totals =  T.sum(self.lin_output1, axis=1).dimshuffle(0,'x') #, T.sum(self.lin_output2,axis=1).dimshuffle(0,'x'))
	self.p_y_given_x = self.lin_output1 / totals #T.concatenate([self.lin_output1/totals, self.lin_output2/totals],axis=1) 
        self.get_py = theano.function(inputs=[input], outputs=self.p_y_given_x) 
...
def load_data(dataset):
    ...
    #the num of rows in the input. It should give the target
    ...
    def shared_dataset(data_xy, borrow=True):
        ...
        # we need them as ints (we use labels as indexl, and if they are
...
def sgd_optimization_mnist(learning_rate=0.13, n_epochs=1000,
    ...
    #test_print = theano.function(inputs=[index], outputs=classifier.probs, givens ={x,y})
